...
export class FavorisDetailPage implements OnInit {
            ...
            buttons: [
                ...
                },
                {
                    ...
                    handler: (data: any) => {
                        const note = {
                            id: 0,
                            noteUser: this.currentUser.idUsers,
                            noteResource: idRessource,
                            noteDate: this.now,
            ...
            buttons: [
                ...
                },
                {
                    ...
                    handler: (data: any) => {
                        const sendNote = {
                            id: note.id,
                            noteUser: note.noteUser.idUsers,
                            noteResource: note.noteResource.id,
                            dateNote: this.now,
                            noteDesc: data.noteDesc,
                            noteStatus: 1
                        };
                        //  note.descNote = data.descNote;
                        // note.dateNote = this.now;
                        // tslint:disable-next-line:triple-equals
                        if (data.noteDesc != '') {
                            this.noteService.addNote(sendNote)
                                .subscribe(not => {
                                        this.getAllNotes();
    ...
    async updateComment(comment: any) {
        ...
        const alert = await this.alertController.create({
            cssClass: 'comment',
            header: 'Modifier votre commentaire',
            // subHeader: 'Enlever cette ressource des favoris?',
            // message: 'Enlever cette ressource des favoris?',
            inputs: [
                {
            ...
            buttons: [
                ...
                },
                {
                    ...
                    handler: (data: any) => {
                        const sendComment = {
                            id: comment.id,
                            commentUser: comment.commentUser.idUsers,
                            commentRessource: comment.commentResource.id,
                            // session: this.currentSession.sessNom,
                            commentDate: this.now,
                            commentDesc: data.commentDesc,
                            commentStatut: 1
                        ...
                        // tslint:disable-next-line:triple-equals
                        if (data.commentDesc != '') {
                            this.commentService.updateComment(sendComment)
                                .subscribe(comm => {
                                        this.getCommentsByResource();
    ...
    async showShareOptions() {
        const modal = await this.modalController.create({
            ...
            componentProps: {url: this.resource.ressUrl, nom: this.resource.ressDesc, user: this.currentUser}
        });
