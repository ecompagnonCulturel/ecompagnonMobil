...
// Question mère SLide
            ...
            await this.questionnaireService.addReponse(responseArray, this.currentUser.CPUsers, questionnaire.id,
               questionnaire.type, repQ1T0)
                .subscribe(appuser => {
                        this.taille = 0;
                        //  console.log(appuser);
        ...
        if (typeQ != null) {
            ...
            questionQuestionnaires.forEach(value => {
                ...
                if ((typeQ === 'T0') && (value.question.questOrdre === 1)) {
                    const contenuArray: any = [];
                    //Affichage des années en fonction de l'année de la session en cours
                    const sessionYear = this.utilService.fromStringToNumber(this.currentSession.sessYear);
                    contenuArray.push({id: 1, value: (sessionYear - 2)});
                    contenuArray.push({id: 2, value: (sessionYear - 1)});
                    contenuArray.push({id: 3, value: sessionYear});
                    value.question.questModalite = contenuArray;
                } else {
            ...
            await this.questionnaireService.getGeneralQuestT0(typeQ, 1)
                .subscribe(quest => {
                    ...
                    quest.forEach(value => {
                        // Première question de T0
                        if ((typeQ === 'T0') && (value.question.questOrdre === 1)) {
                            //Affichage des années en fonction de l'année de la session en cours et du mois en cours
